...
//   }
//
//   // // List<MessageModel> parseGetChat(String responseBody) {
//   // //   final parsed = json
//   // //       .decode(responseBody)['Response']['Records']['Messages']
...
//   // // }
//   var data;
//   Stream<List> getGetChat(String username) async* {
//     String finalUrl = '$baseurl/User/GetMessageList';
//
//     print("username $username lang $lang token $token");
...
//   }
//
//   Stream<List> getTranslatedChat(String chatId, String? langCode) async* {
...
class Apis {
  ...
  //     yield Message.fromJson(
  //         jsonDecode(response.body)['Response']['Records'][0]['Messages']);
  //   } else {
  //     throw Exception('Unable to fetch products from the REST API');
  //   }
  // }

  // Stream GetMessageList() async* {
  //   while (true) {
  //     await Future.delayed(Duration(milliseconds: 500));
  //     Stream someProduct = getGetChat();
  //     Get.find<MessageController>().updateBooking(data);
  //     yield* someProduct;
  //   }
  // }
  ...
  Future TranslateChat(String chatId, String? langCode) async {
    print("chatid $chatId langcode $langCode base $baseurl token $token");
    String finalurl = '$baseurl/User/TranslateChat/$chatId';
    var response = await http.post(Uri.parse(finalurl),
        headers: {'lang': langCode!, 'Token': token!});
    if (response.statusCode == 200) {
